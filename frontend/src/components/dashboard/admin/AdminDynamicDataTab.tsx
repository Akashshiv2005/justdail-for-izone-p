...
export default function AdminDynamicDataTab({ tab, onOpenSidebar }: { tab: string, onOpenSidebar: () => void }) {
  ...
  // Load reviews for a specific business when user clicks into it
  ...
  const handleDeleteReview = async (reviewId: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Update the review count in rows
        if (selectedBusiness) {
  ...
  React.useEffect(() => {
    // Reset reviews drill-down when tab changes
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-black text-slate-800">
                  Reviews — <span className="text-blue-600">{selectedBusiness.name}</span>
                </h2>
              </div>
                    ...
                    <div className="flex items-start gap-4 mb-4">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
                          <MapPin size={11} /> {biz.city} · {biz.category}
                        </p>
                      </div>
                    </div>
