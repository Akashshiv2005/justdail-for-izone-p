...
export default function ServicesTab({ data, editingRow, setEditingRow, businessId, refreshData }: any) {
  return (
    <motion.div 
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="relative bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
              ...
              <div className="flex justify-between mt-2">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditingRow(null)} className="px-4 py-2 text-slate-600 hover:bg-slate-100 rounded-lg transition-colors font-medium">Cancel</button>
                  <button onClick={() => {
                      if (!editingRow.col1 || editingRow.col1.trim() === '') {
                          alert('Service name is required');
                          return;
                      }
                      const payload = {
                          custom_name: editingRow.col1,
